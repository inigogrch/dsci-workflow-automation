...
After removing missing values from the @tbl-cleaned-adult-dataset, we randomly sample 10% of the data (which contains a total of `{r} n_obs` observations), bringing our sample size to `{r} sample_size` data points. The sample is then split into training and testing sets (80-20 split) for prediction analysis.
...
Above, we can see that the @tbl-cleaned-adult-dataset has been successfully split, with our @tbl-training-adult-dataset containing `{r} n_training` rows, representing about 80% of our sample size: `{r} sample_size`.
...
Using our @tbl-training-adult-dataset with the irrelevant columns dropped, we create pairwise plots to examine relationships between continuous variables (`age`, `hours_per_week`, `education_num`) and the response variable, as well as associations among the input variables.

![Pairwise Plot of Response and Predictors](../results/eda/pairwise_plot.png){#fig-pairwise-plot}


The @fig-pairwise-plot shows that `age` is right-skewed, `hours_per_week` peaks around 40, and `education_num` has a bimodal distribution. Weak correlations (< 0.6) suggest minimal multicollinearity.
...
In the following code, we fit the logistic regression model to the @tbl-training-adult-dataset using the relevant predictors. 
...
![ROC Curve of the Logistic Regression Model](../results/model/roc_plot.png){#fig-roc-curve}
...
![Confusion Matrix of Full Model on Testing Set](../results/model/conf_matrix_plot.png){#fig-confusion-plot}
...
From our @tbl-classification-results, we observe the following metrics:
1. **Sensitivity (SN): `{r} sensitivity`%** - The model correctly identifies `{r} sensitivity`% of higher-income individuals.
2. **Specificity (SP): `{r} specificity`%** - `{r} specificity`% of lower-income individuals are correctly classified.
3. **Precision (PR): `{r} precision`%** - `{r} precision`% of predicted >50K individuals actually earn >50K, indicating many false positives.
4. **Accuracy (ACC): `{r} accuracy`%** - `{r} accuracy`% of overall predictions are correct.
5. **Cohen's Kappa (κ): `{r} kappa`%** - Moderate agreement, better than random chance but room for improvement.
